(function ($) {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: (data, type, row, meta) => {
                    return [
                        `   <button type="button" class="btn btn-sm bg-secondary edit-items" data-slide-id="${row.id}" >`,
                        `       <i class="fas fa-pencil-edit"></i> ${l('SlideItems')}`,
                        '   </button>',
    ...
    $(document).on('click', '.edit-user', function (e) {
        ...
        e.preventDefault();
        abp.ajax({
            url: abp.appPath + 'Slide/EditModal?id=' + userId,
            type: 'POST',
            dataType: 'html',
            success: function (content) {
                $('#SlideEditModal div.modal-content').html(content);
            },
            error: function (e) { }
        });
    });

    $(document).on('click', 'a[data-target="#SlideModal"]', (e) => {
        console.log(e);
